import {getImageTaskResult, getTaskResult} from '../api/generateResponse';

export const pollImageResult = async (rootUrl, taskId, handleResult, handleError) => {
        ...
        try {
            ...
            if (result && result.data) {
                if (result.data.state === 'PENDING' || result.data.state === 'RETRY') {
                    setTimeout(async () => {
                        try {
                            const nextResult = await pollImageResult(rootUrl, taskId, handleResult, handleError);
                            resolve(nextResult);
                        } catch (error) {
                            reject(error);
                        }
...
export const pollResult = async (rootUrl, taskId, handleResult, handleError) => {
    return new Promise(async (resolve, reject) => {
        try {
            const result = await getTaskResult(rootUrl, taskId);
            if (result && result.data) {
                if (result.data.state === 'PENDING' || result.data.state === 'RETRY') {
                    setTimeout(async () => {
                        try {
